// LOCATION: src/pages/admin/AdminPlans.jsx
    ...
    <div className="adm-modal-overlay" onClick={onClose}>
      <div className="adm-modal" style={{ maxWidth: 560 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="form-group" style={{ marginTop: '1rem' }}>
          ...
          <input className="form-control" placeholder="e.g. Starter Plan"
...
export default function AdminPlans() {
  ...
  const [modalPlan, setModalPlan] = useState(undefined); // undefined = closed, null = new, object = edit

  const fetchAll = () => {
    setLoading(true);
    Promise.all([adminService.getPlans(), adminService.getSectors()])
  ...
  const togglePlanStatus = async (plan) => {
    try {
      await adminService.updatePlan(plan.id, { status: plan.status === 'active' ? 'inactive' : 'active' });
    ...
    <div className="adm-page">
      ...
      <div className="adm-card">
        ...
        {loading ? (
          ...
          <div className="adm-empty"><p>No plans yet. Create your first plan.</p></div>
        ) : (
          <div className="table-wrap">
            <table>
              <thead>
                <tr>
                  <th>Plan Name</th><th>Sector</th><th>Min Amount</th><th>Max Amount</th>
                  <th>ROI</th><th>Duration</th><th>Featured</th><th>Status</th><th>Actions</th>
                </tr>
              </thead>
              <tbody>
                {plans.map(p => (
                  <tr key={p.id}>
                    ...
                    <td>{p.is_featured ? <span className="badge badge-gold"><FiStar size={11}/> Yes</span> : '—'}</td>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: '0.35rem', flexWrap: 'wrap' }}>
                        ...
                        <button className="btn btn-danger btn-sm" onClick={() => handleDelete(p)}><FiTrash2 size={13} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
